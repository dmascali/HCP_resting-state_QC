function TCs = RestingStateStats_mod(motionparameters,hp,TR,ICAs,noiselist,wbcommand,inputdtseries,bias,outprefix,dlabel,bcmode,outstring,WM,CSF)
% DM edits: 1) Returns all possible MGT, WM and CSF at each stage
%           2) Calculate DVARS at each stage
%           3) Disalbed GrayPlots
%           4) Don't save any image
%           5) Still saves the stats file (not used, just to check everything is
%           ok)
%
...
SaveVarianceNormalizationImage = 0; %If non-zero, output map of sqrt(UnstructNoiseVar)
%SaveMGT = 1; %If non-zero, save mean grayordinate time series at each stage
SaveGrayOrdinateMaps = 0; % Set to non-zero to save a number of grayordinate maps (into a single .dtseries.nii)
...
%   dlmwrite([outprefix '_NoiseMGT.txt'],NoiseMGT);
  TCs.OrigMGT = OrigMGT;
  TCs.HighPassMGT = HighPassMGT;
  TCs.PostMotionMGT = PostMotionMGT;
  TCs.CleanedMGT = CleanedMGT;
  TCs.UnstructNoiseMGT = UnstructNoiseMGT;
  TCs.NoiseMGT = NoiseMGT;
end

%dlmwrite([outprefix '_CleanedMGT.txt'],CleanedMGT);
if ~strcmp(WM,'NONE')
    %dlmwrite([outprefix '_CleanedWMtc.txt'],WMtcClean);
    TCs.WMtcOrig = WMtcOrig;
    TCs.WMtcHP   = WMtcHP;
    TCs.WMtcPM   = WMtcPM;
    TCs.WMtcClean= WMtcClean;
end
if ~strcmp(CSF,'NONE')
    %dlmwrite([outprefix '_CleanedCSFtc.txt'],CSFtcClean);
    TCs.CSFtcOrig = CSFtcOrig;
    TCs.CSFtcHP   = CSFtcHP;
    TCs.CSFtcPM   = CSFtcPM;
    TCs.CSFtcClean= CSFtcClean;
end
...
% Compute DVARS
[TCs.OrigDV,TCs.OrigSD]                   = calculate_dvars_sd(OrigTCS);
[TCs.HighPassDV,TCs.HighPassSD]           = calculate_dvars_sd(HighPassTCS);
[TCs.PostMotionDV,TCs.PostMotionSD]       = calculate_dvars_sd(PostMotionTCS);
[TCs.CleanedDV,TCs.CleanedSD]             = calculate_dvars_sd(CleanedTCS);
[TCs.UnstructNoiseDV,TCs.UnstructNoiseSD] = calculate_dvars_sd(UnstructNoiseTCS);
[TCs.NoiseDV,TCs.NoiseSD]                 = calculate_dvars_sd(NoiseTCS);
...
function [DV,SD] = calculate_dvars_sd(img)
